...
DATASET = Path(
    r"D:\Research\SPRSound-main\SPRSound-main\BioCAS2022"
)

json_folder = DATASET / "train2022_json"
wav_folder = DATASET / "train2022_wav"

json_files = sorted(json_folder.glob("*.json"))

print("Number of recordings:", len(json_files))
...
with open(json_file, "r") as f:
    annotation = json.load(f)
        ...
        ## slicing the signal
       for event in annotation["event_annotation"]:
    ...
    signal_event = signal[start_sample:end_sample]

    if len(signal_event) == 0:
        continue
